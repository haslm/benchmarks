...
from .modeling import predict_in_batches


def get_errors( config, y_test, y_hat, smoothed=True):
    ...
    Args:
        ...
        y_test (np.ndarray): array of test targets corresponding to true values to be predicted at end of each sequence
        y_hat (np.ndarray): predicted test values for each timestep in y_test
...
def process_errors( config, y_test, y_hat, e_s):
    """
    Args:
        config (Config): configuration object
    ...
    for i in range(1, num_windows+2):
        ...
        window_anom_indices = get_anomalies(config,window_e_s, window_y_test, epsilon, i-1, i_anom, len(y_test))
...
def compare_to_epsilon(config,e_s, epsilon, len_y_test, inter_range, chan_std, 
...
def prune_anoms(config, E_seq, e_s, non_anom_max, i_anom):
...
def get_anomalies(config,e_s, y_test, z, window, i_anom_full, len_y_test):
    ...
    E_seq, i_anom, non_anom_max = compare_to_epsilon(config, e_s, epsilon, len_y_test,
    ...
    E_seq_inv, i_anom_inv, inv_non_anom_max = compare_to_epsilon(config,e_s_inv, epsilon_inv, 
        len_y_test, inter_range, chan_std, std, config.error_buffer, window, i_anom_full)

    if len(E_seq) > 0:
        i_anom = prune_anoms(config, E_seq, e_s, non_anom_max, i_anom)

    if len(E_seq_inv) > 0:
        i_anom_inv = prune_anoms(config, E_seq_inv, e_s_inv, inv_non_anom_max, i_anom_inv)
...
def find_error_sequences(config, model, x_test, y_test):
    """
    Args:
        config (Config): 配置文件
        model (Sequence): 预训练的模型序列
        x_test (np_array): 输入的序列
        y_test (np_array): 真实的输出序列

    Returns:
        E_seq (list of tuples): contains start and end indices of anomalous ranges
        anom_scores(list of float) 估计的异常数据得分
        y_hat(np array) 预测的序列
    """
    y_hat = predict_in_batches(config,model,x_test)
    e_s = get_errors(config,y_test,y_hat)
    E_seq, anom_scores = process_errors(config,y_test,y_hat,e_s)
    return E_seq, anom_scores, y_hat